window.onload = function() {
	...
	function promptChoices(dataParameter) {
		...
		video.addEventListener("timeupdate", function(questionData) {
			...
			/*if (video.currentTime <= loseTime - 0.2 && video.currentTime >= loseTime - 0.5 && parseInt(oldPrice) > 0) {
				video.currentTime = winTime;
			};
			if (parseInt(oldPrice) <= 0) {
				progressText.style.opacity = 0;
				progressText.innerHTML = "$1";
				video.pause();
